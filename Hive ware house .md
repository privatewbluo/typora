...
nodemanage 是节点上的资源和任务管理器。nodemanage 会定时向resourcemanage告知本节点资源（CPU，内存）的使用情况。
...
map的个数 由split 决定，具体详情<span style='background-color:orange'>**查看split** </span>
...
- 更改location
  ...
  alter table zeta_dev_clsfd_working.clsfd_ad_cube_daily_w set location "hdfs://hercules/sys/edw/zeta_dev/working/clsfd/clsfd_working/clsfd/clsfd_ad_cube_daily_w/"; <span style='background-color:lightblue'>**--要写绝对路径**</span>
...
<a href='https://blog.csdn.net/strongyoung88/article/details/81156271'>谓词下推 </a>&<a href='http://bigdatums.net/2017/08/29/what-is-predicate-pushdown/'>ppd</a>
...
<span style='background-color:lightgreen'> **set hive.default.fileformat   : 默认为textfile**</span>
...
- Namenode and Datanode 
  ...
  - Namenode  (master 节点)
    ...
    2. 两个文件：fsimage :元数据镜像
       ...
       存放在namenode  中：由/etc/hadoop/hdfs-site.xml 中的 

       dfs.namenode.name.dir:

       ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_157.png)

       ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_158.png)
...
![](C:\Users\wenbluo\Desktop\wbluo\hive\p_171.png)
...
- -cat   ###输出文本  <span style='background-color:lightgreen'>**适用于 textinput format** </span>
...
- -text   :<span style='background-color:lightgreen'>**当文件并不是text input file   ,比如：sequence file .**</span>

  ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_132.png)
...
- -du -df
   ...
   2. -df ：display  capacity ,free and  used space of the filesystem
...
- hdfs  dfsadmin -report : 查看集群状态
   ...
   ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_144.png)
...
fsck :<span style='background-color:lightblue'>**file status check** </span>

hadoop fs -ls /usr/hive/warehouse/p_clsfd_t.db/wenbluo01/000000_0  ;

![](C:\Users\wenbluo\Desktop\wbluo\hive\p_154.png)

- 文件副本个是 1
- 文件大小：9kb

hdfs fsck  /user/hive/warehouse/p_clsfd_t.db/wenbluo01/000000_0 -files -blocks -locations 

![](C:\Users\wenbluo\Desktop\wbluo\hive\p_155.png)
...
- <span style='background-color:yellow'>**msck table tablename :检查metestore**</span>
  ...
  **msck repair  table tablename**   <span style='background-color:orange'>**针对分区表**</span>
  ...
  ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_134.png)

  -->怎么解决listing the partitions that were not in the metastore but not actually adding them ?

  1. alter table xxxx add partition(clsfd_site_id=3001)手动添加分区
...
- msck repair table : 仅仅更新metadata  与 hdfs 文件映射关系。
  ...
  1. background :
     ...
     -->Metadata 逻辑隐射关系怎么改动，应该是更改hdfs 上文件把。

     ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_142.png)

     及时显式删除hdfs上文件，底层metadata 依旧保持不变，仍然有0927/0928分区。![](C:\Users\wenbluo\Desktop\wbluo\hive\p_143.png)

     

  2. 因此要做alter table drop partition ，更改metadata.
  ...
  3. ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_113.png)
     ...
     summary  :<span style='background-color:pink'>**可以看到msck  指令是负责(添加)映射关系，并不能底层更改(删除)schema** </span>
...
2. <a href='https://cwiki.apache.org/confluence/display/Hive/Hive+on+Tez'>Hive on Tez</a>
   ...
   - ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_63.png)
     ...
     4. ![](C:\Users\wenbluo\Desktop\wbluo\hive\p_175.PNG)

     -   Tez 减少中间hdfs 文件io 操作，通过将中间缓存到内存。
...
-    snapshot
  ...
  /sys/edw/.snapshot/snapshot-sysedw-20200223/clsfd/clsfd_tables/clsfd/ 

  1. <a href='https://docs.cloudera.com/runtime/7.0.3/data-protection/topics/hdfs-considerations-for-working-with-hdfs-snapshots.html'> create a snapshot</a>

       

     ```
     for example ,consider the directory /foo that is enabled to create a  snapshot. For the directory /foo with a snapshot snap1 ,the path /foo/.snapshot/snap_1 refers to /foo
     ```
...
https://www.cnblogs.com/drunkhero/p/strenghen-comprehension-on-dirty-read-and-phantom-of-mysql.html
...
![](C:\Users\wenbluo\Desktop\wbluo\hive\p_131.png)

- <span style='background-color:lightblue'>**中途中断，并不满足A（Atomicity）操作**</span>，事务开始后作业，要么不做，要么做完。中断程序，直接回滚到原始提交事务前，初始状态，所有操作当作没有发生

  -->但是 hive & spark-sql 并不满足A， 仍旧有部分文件已经被修改。
...
set mapreduce.map.java.opts=-Xmx40960m;
set mapreduce.reduce.java.opts=-Xmx40960m;
set hive.exec.parallel=true;
set hive.map.aggr = true;
set hive.groupby.skewindata=true;
set hive.groupby.orderby.position.alias=true;
set hive.optimize.skewjoin=true;
ALTER TABLE kylin_intermediate_clsfd_ad_cube_dd SET TBLPROPERTIES('auto.purge'='true'); --- auto.purge ？
